...
# KPI 3: Preo médio novo (em reais)
average_new_price = df['new_price'].mean()
col3.metric(label='Preço Médio (R$)', value=f'{average_new_price:.2f}')

# Quais marcas são mais encontradas até a 5º pagina
st.subheader('Marcas mais encontradas até a 5º pagina')
col1, col2 = st.columns([4,2])

top_5_pages_brands = df['brand'].value_counts().sort_values(ascending=False)
col1.bar_chart(top_5_pages_brands)
col2.write(top_5_pages_brands)
